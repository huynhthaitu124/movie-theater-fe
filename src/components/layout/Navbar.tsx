...
const Navbar: React.FC = () => {
  ...
  return (
    <nav className="bg-white dark:bg-secondary-800 shadow-md">
          ...
          <div className="hidden md:flex items-center space-x-4">
            ...
            {isAuthenticated ? (
              <div className="relative ml-3 flex items-center space-x-3">
                <Link to={currentUser?.role === 'admin' ? '/admin/dashboard' : '/account'} className="flex items-center space-x-1">
                  <User size={20} />
                  <span>{currentUser?.name}</span>
                </Link>
                <button
                  onClick={handleLogout}
                  className="text-secondary-800 dark:text-secondary-200 hover:text-accent-600 dark:hover:text-accent-400 flex items-center"
                >
      ...
      {/* Mobile menu */}
      {isMenuOpen && (
        <div className="md:hidden bg-white dark:bg-secondary-800 shadow-lg animate-fade-in">
