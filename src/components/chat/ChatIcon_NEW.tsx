...
const ChatIcon: React.FC = () => {
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    try {
      // Tạm thời để member chat với support team
      // Trong thực tế cần API để lấy admin ID thực tế
      const supportTeamId = currentUser?.accountid; // Tạm thời sử dụng accountid của user
      if (!supportTeamId) {
        throw new Error('Unable to identify user. Please login again.');
      }
      
      await sendMessage(messageInput, supportTeamId);
      setMessageInput('');
    } catch (error: any) {
